...
from mist._backup_modules.block_backup import Block
from mist._backup_modules.embedding import BertEmbeddings
from mist.modules.mha import MHA, ParallelMHA
from mist.modules.mlp import FusedMLP, Mlp, ParallelFusedMLP, ParallelMLP

from mist.modules.fused_dense import FusedDense
from mist.model import layer_norm as layer_norm_module
...
def create_mixer_cls(
    ...
    if process_group is not None:
        ...
        assert not cross_attn, "cross_attn is not supported in parallel mode"
        assert not fused_bias_fc, "fused_bias_fc is not supported in parallel mode"
        assert not return_residual
    else:
        other_kwargs["cross_attn"] = cross_attn
        other_kwargs["fused_bias_fc"] = fused_bias_fc
        other_kwargs["return_residual"] = return_residual
...
def create_mlp_cls(
    ...
    if process_group is not None:
        parallel_kwargs["process_group"] = process_group
        assert not return_residual, "return_residual is not supported in parallel mode"
    else:
        other_kwargs["return_residual"] = return_residual
    if fused_mlp:
...
def create_block(config, layer_idx=None, process_group=None, device=None, dtype=None):
    ...
    # Zhanda [2024-02-26] Remove residual
    ...
    mixer_cls = create_mixer_cls(
        config, cross_attn, return_residual=return_residual, **common_kwargs
    )
    mlp_cls = create_mlp_cls(
        config, layer_idx, return_residual=return_residual, **common_kwargs
    )
...
class BertEncoder(nn.Module):
    def __init__(
        self, config: BertConfig, process_groups=None, device=None, dtype=None
    ):
        super().__init__()
        self.use_flash_attn = getattr(config, "use_flash_attn", False)
        if not isinstance(process_groups, (list, tuple)):
            process_groups = [process_groups] * config.num_hidden_layers
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        if self.fused_dropout_add_ln and layer_norm_module is None:
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if not self.fused_dropout_add_ln:
            ...
            hidden_states = layer_norm_module.layer_norm(
                hidden_states,
...
class BertModel(BertPreTrainedModel):
    def __init__(
        ...
        self.encoder = BertEncoder(
            config, process_groups=process_groups, device=device, dtype=dtype
        )
...
class BertForPreTraining(BertPreTrainedModel):
    def __init__(
        ...
        # ========================================================================
        # Added by Zhanda
        self.only_return_loss = getattr(config, "only_return_loss", False)
        self.uses_fp32_loss = getattr(config, "uses_fp32_loss", True)
        # ========================================================================
    ...
    def forward(
        ...
        if labels is not None and next_sentence_label is not None:
            if (
                ...
                _prediction_scores = prediction_scores.reshape(
                    -1, prediction_scores.size(-1)
                )
            ...
            _seq_relationship_score = seq_relationship_score.reshape(
                -1, seq_relationship_score.size(-1)
            )
            _next_sentence_label = next_sentence_label.reshape(-1)
            next_sentence_loss = self.nsp_loss(
                _seq_relationship_score, _next_sentence_label
            )
            if self.uses_fp32_loss:
                total_loss = masked_lm_loss.float() + next_sentence_loss.float()
            else:
                total_loss = masked_lm_loss + next_sentence_loss

            if self.only_return_loss:
                return total_loss
...
if __name__ == "__main__":
    ...
    model = BertForPreTraining(
        config, process_groups=dist.group.WORLD, device=device, dtype=torch.float16
    )
    model = model.to(device)
    model = model.half()
    print(
        f"[{local_rank}] Peak Allocated Memory: {torch.cuda.max_memory_allocated() / 1024 ** 2:.4f}MB"
    )
    input_ids = torch.randint(0, 100, (8, 512), device=device)
    labels = torch.randint(0, 100, (8, 512), device=device)
    next_sentence_label = torch.randint(0, 2, (8,), device=device)
    print(model(input_ids, labels=labels, next_sentence_label=next_sentence_label))
