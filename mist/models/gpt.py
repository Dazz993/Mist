...
from mist.distributed.op import (
    all_gather_raw,
...
from mist._backup_modules.block_backup import Block, ParallelBlock
from mist._backup_modules.embedding import GPTEmbeddings, ParallelGPTEmbeddings
from mist.modules.fused_dense import ColumnParallelLinear
from mist.modules.mha import MHA, ParallelMHA
from mist.modules.mlp import (
    FusedMLP,
    GatedMlp,
    Mlp,
    ParallelFusedMLP,
...
from mist.model import layer_norm
...
from mist.modules import rms_norm
...
def create_block(config, layer_idx=None, process_group=None, device=None, dtype=None):
    ...
    norm_cls = partial(
        nn.LayerNorm if not use_rms_norm else rms_norm.RMSNorm,
...
class GPTModel(GPTPreTrainedModel):
    def __init__(
        ...
        self.tensor_parallel = (
            any(pg is not None for pg in process_groups)
            or pre_post_process_group is not None
        )
        ...
        if pre_post_process_group is None:
            ...
            self.embeddings = ParallelGPTEmbeddings(
                config.hidden_size,
                vocab_size,
                config.max_position_embeddings,
                process_group=pre_post_process_group,
        ...
        if self.fused_dropout_add_ln:
            if (
                not self.parallel_block and layer_norm.dropout_add_layer_norm is None
            ) or (
                self.parallel_block
                and layer_norm.dropout_add_layer_norm_parallel_residual is None
            ):
                raise ImportError("dropout_layer_norm is not installed")
        if self.prenorm:
            self.drop_f = nn.Dropout(config.resid_pdrop)
            norm_cls = nn.LayerNorm if not use_rms_norm else rms_norm.RMSNorm
            self.ln_f = norm_cls(
                config.hidden_size, eps=config.layer_norm_epsilon, **factory_kwargs
            )
        if pre_post_process_group is not None and self.prenorm:
    ...
    def tie_weights(self):
        if self.pre_post_process_group is not None:
            sync_shared_params(self, self.pre_post_process_group)
    ...
    def forward(self, input_ids, position_ids=None, inference_params=None):
        ...
        mixer_kwargs = (
            ...
            if self.tensor_parallel is not None and self.sequence_parallel
            else {}
        )
        ...
        if self.prenorm:
            if not self.fused_dropout_add_ln:
                ...
                if not self.parallel_block:
                    fused_add_norm_fn = (
                        rms_norm.dropout_add_rms_norm
                        if isinstance(self.ln_f, rms_norm.RMSNorm)
                        else layer_norm.dropout_add_layer_norm
                    )
                    ...
                    fused_add_norm_fn = (
                        ...
                        if isinstance(self.ln_f, rms_norm.RMSNorm)
                        else layer_norm.dropout_add_layer_norm_parallel_residual
                    )
...
class GPTLMHeadModel(GPTPreTrainedModel, GenerationMixin):
    def __init__(
        ...
        self.transformer = GPTModel(
            config, process_groups=process_groups, **factory_kwargs
        )
        ...
        if pre_post_process_group is None:
            self.lm_head = nn.Linear(
                embed_dim, vocab_size, bias=lm_head_bias, **factory_kwargs
            )
        else:
        ...
        # Move to before forward() to avoid communication between meta tensors
        # self.tie_weights()
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # Note(zhanda): using vocab_parallel_cross_entropy anyway to save memory
            loss = vocab_parallel_cross_entropy(
                lm_logits, labels, self.pre_post_process_group
            )
            # Here loss is not reduced, so we need to reduce it
            loss = torch.mean(loss)
            return loss
            # if (
            #     self.pre_post_process_group is not None
            #     and self.pre_post_process_group.size() > 1
            # ):
            #     loss = vocab_parallel_cross_entropy(
            #         lm_logits, labels, self.pre_post_process_group
            #     )
            ...
            #     loss = torch.mean(loss)
            # else:
            #     # TODO(zhanda): use cross_entropy_loss from triton if possible
            #     # loss_fn = cross_entropy.cross_entropy_loss
