...
def del_mem_buff(name):
    """Delete the memory buffer."""
    del _MEM_BUFFS[name]


def allocate_ring_mem_buff(name, num_buffers, numel, dtype, track_usage):
    """Allocate a ring of memory buffers."""
    assert name not in _RING_MEM_BUFFS, "memory buffer {} already allocated.".format(
        name
    )
    _RING_MEM_BUFFS[name] = RingMemBuffer(name, num_buffers, numel, dtype, track_usage)
    return _RING_MEM_BUFFS[name]


def get_ring_mem_buff(name):
    """Get the memory buffer."""
    return _RING_MEM_BUFFS[name]


def del_ring_mem_buff(name):
    """Delete the memory buffer."""
    del _RING_MEM_BUFFS[name]


def get_all_mem_buff_numel():
    """Get the memory buffer."""
    return sum([buff.numel for buff in _MEM_BUFFS.values()])
