...
# This file is based on transformers.utils.fx (transformers==4.29.2)
...
from transformers import PretrainedConfig, PreTrainedModel, logging
...
def torch_nn_functional_gelu(x):
    return x
...
def torch_softmax(input, dim, *, dtype=None) -> torch.Tensor:
    if dtype is not None:
        return input.to(dtype=dtype)
    return input


def torch_nn_functional_softmax(
    input: torch.Tensor,
    dim: Union[int, NoneType] = None,
    _stacklevel: int = 3,
    dtype: Union[int, NoneType] = None,
) -> torch.Tensor:
    if dtype is not None:
        return input.to(dtype=dtype)
    return input
...
def torch_tensor_mean(input, dim=None, keepdim=False):
    raise NotImplementedError(
        f"Only used for signature analysis for now. Add implementation if needed."
    )
...
def torch_tensor_add(input, other, *, alpha=1):
...
_MANUAL_META_OVERRIDES: Dict[Callable, Callable] = {
    # torch functions or nn.functional
    ...
    torch.add: torch_add,
    torch.Tensor.add: torch_tensor_add,
    torch.mul: torch_mul,
    ...
    torch.Tensor.mean: torch_tensor_mean,
...
def _gen_constructor_wrapper(target):
    """
    This helper function wraps the target function for it to be patched. If any input is a Proxy, it will call corresponding tracer's create_proxy funxtion to create a new proxy. Otherwise, it will call the original function.

    Parameters
    ----------
    target
        Target function to be wrapped and patched.
    """
...
class HFTracer(Tracer):
    ...
    def _generate_dummy_input(
        model: PreTrainedModel, input_name: str, shape: List[int]
        ...
        elif "next_sentence_label" in input_name:
            inputs_dict[input_name] = torch.zeros(
    ...
    def trace(
        ...
        for input_name in input_names:
            ...
            if isinstance(root, self.supported_archs) or type(
                root
            ).__qualname__.startswith("_deserialize_graph_module"):
