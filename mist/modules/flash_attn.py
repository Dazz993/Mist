...
class FlashAttnQKVPackedFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, qkv, dropout_p, softmax_scale, causal, window_size, return_softmax
    ):
...
class FlashAttnKVPackedFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, q, kv, dropout_p, softmax_scale, causal, window_size, return_softmax
    ):
        if softmax_scale is None:
...
class FlashAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, q, k, v, dropout_p, softmax_scale, causal, window_size, return_softmax
    ):
        if softmax_scale is None:
            softmax_scale = q.shape[-1] ** (-0.5)
        out, q, k, v, out_padded, softmax_lse, S_dmask, rng_state = _flash_attn_forward(
...
torch.fx.wrap("flash_attn_qkvpacked_func")
torch.fx.wrap("flash_attn_kvpacked_func")
torch.fx.wrap("flash_attn_func")
torch.fx.wrap("flash_attn_varlen_qkvpacked_func")
torch.fx.wrap("flash_attn_varlen_kvpacked_func")
torch.fx.wrap("flash_attn_varlen_func")
