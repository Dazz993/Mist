...
def bias_gelu_impl(input, bias):
    return GeLUFunction.apply(input, bias)
...
def fast_gelu_impl(input):
    return FastGeLUFunction.apply(input)
...
class SwiGLUFunction(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        # Note(zhanda): here we can not directly return swiglu_bwd(x, y, dout)
        # have to unpack the tuple and return. Otherwise, there may be a
        # issue with the autograd engine when calling `torch.autograd.grad`.
        dx, dy = swiglu_bwd(x, y, dout)
        return dx, dy
...
torch.fx.wrap("fast_gelu_impl")
torch.fx.wrap("swiglu")
