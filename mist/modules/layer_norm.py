...
class DropoutAddLayerNormFn(torch.autograd.Function):
    ...
    def forward(
        ...
        if not return_dmask:
            zmat = make_viewless_tensor(
                zmat.view(x0.shape), requires_grad=zmat.requires_grad, keep_graph=False
            )
            if not prenorm:
                return zmat
            else:
                xmat = make_viewless_tensor(
                    xmat.view(x0.shape),
                    requires_grad=xmat.requires_grad,
                    keep_graph=False,
                )
                return zmat, xmat
...
torch.fx.wrap("dropout_add_layer_norm")
torch.fx.wrap("dropout_add_layer_norm_subset")
torch.fx.wrap("dropout_add_layer_norm_parallel_residual")
