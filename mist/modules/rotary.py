...
class ApplyRotaryEmbQKV_(torch.autograd.Function):
    ...
    def forward(
        ...
        if isinstance(seqlen_offsets, int):
            ...
            ctx.seqlen_offsets = None
        ctx.interleaved = interleaved
        # FIXME(zhanda): IDK, if we don't do this, then the qkv's symbolic info will be lost
        qkv = qkv + 0
        return qkv
...
def apply_rotary_emb_qkv_(
    ...
    return ApplyRotaryEmbQKV_.apply(
        qkv, cos, sin, cos_k, sin_k, interleaved, seqlen_offsets
    )
...
torch.fx.wrap("apply_rotary_emb_func")
torch.fx.wrap("apply_rotary_emb")
torch.fx.wrap("apply_rotary_emb_qkv_")
torch.fx.wrap("apply_rotary_emb_kv_")
