...
debugging_counter_fwd = 0
debugging_counter_bwd = 0
...
class FusedDenseFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        """
        # print(f"[DEBUG FusedDenseFunc]: {x.shape=}, {weight.shape=}, {x.float().sum().item():.6f}, {weight.float().sum().item():.6f}")
        ...
        if process_group is not None and sequence_parallel:
            ...
            total_x, handle_x = dist_op.all_gather_raw(x, process_group, async_op=True)
        else:
            total_x = x
        ...
        # =========== Debugging ===========
        # global debugging_counter_fwd
        # debugging_counter_fwd += 1
        # print(f"[RANK={dist.get_rank()}] [DEBUG FusedDenseFunc FWD - {debugging_counter_fwd}], x: {x.float().sum().item():.6f}, w: {weight.float().sum().item():.6f}, o: {output.float().sum().item():.6f}")
        # =========== Debugging ===========
    ...
    def backward(ctx, grad_output, *args):
        ...
        if ctx.compute_weight_gradient:
            ...
            # global debugging_counter_bwd
            # debugging_counter_bwd += 1
            # print(f"{torch.distributed.get_rank()=}, {debugging_counter_bwd=}, {x.float().abs().sum().item()=}")
            if process_group is not None and sequence_parallel:
                total_x, handle_x = dist_op.all_gather_raw(
                    x, process_group, async_op=True
                )
            else:
                total_x = x
        else:
        ...
        if ctx.needs_input_grad[0]:
            ...
            if process_group is not None:
                reduce_fn = (
                    dist_op.reduce_scatter_raw
                    if sequence_parallel
                    else dist_op.all_reduce_raw
                )
        ...
        # global debugging_counter_bwd
        # debugging_counter_bwd += 1
        # print(f"[RANK={dist.get_rank()}] [DEBUG FusedDenseFunc BWD - {debugging_counter_fwd}], grad output: {grad_output.float().sum().item():.6f}, grad weight: {grad_weight.float().sum().item():.6f}, grad_input: {grad_input.float().sum().item():.6f}")
        # ===================================
...
def fused_dense_func(
    ...
    if (
        ...
        # assert process_group is None or process_group.size() == 1 or getattr(process_group, "_is_mist_process_group", False)
    ...
    def __init__(
        ...
        *,
        ...
        if process_group is not None:
            ...
            if out_features % multiple_of != 0:
                raise ValueError(
                    f"out_features ({out_features}) must be a multiple of {multiple_of}"
                )
            dim = dist_op.get_dim_for_local_rank(
                dim=out_features,
        ...
        # Gather_output is accomplished by the same strategy as sequence_parallel
        self.sequence_parallel = sequence_parallel or gather_output
...
class RowParallelLinear(nn.Linear):
    ...
    def forward(self, x):
        ...
        if self.process_group is not None:
            reduce_fn = (
                dist_op.reduce_scatter if self.sequence_parallel else dist_op.all_reduce
            )
...
class FusedMLPFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if heuristic == -1:
            ...
            activation_fn = (
                ...
                else (activations.sqrelu_fwd if activation == "sqrelu" else F.relu)
            )
    ...
    def backward(ctx, grad_output, *args):
        ...
        if checkpoint_lvl in [0, 1]:
            ...
            (bias1,) = rest
            if process_group is not None and sequence_parallel:
                total_x, _ = dist_op.all_gather_raw(x, process_group)
            if ctx.heuristic == -1:
                pre_act = F.linear(total_x, weight1, bias1)
                with torch.jit.fuser("fuser2"):
                    output1 = activation_fn(pre_act)
        ...
        if ctx.heuristic == -1:
            ...
            activation_grad_fn = (
                activations.gelu_bwd
                if activation == "gelu_approx"
                else (
                    activations.sqrelu_bwd
                    if activation == "sqrelu"
                    else activations.relu_bwd
                )
            )
            with torch.jit.fuser("fuser2"):
...
class FusedMLP(nn.Module):
    ...
    def forward(self, x, process_group=None):
        ...
        if self.return_residual:
            out, x = out
        if process_group is not None:
            out = dist_op.reduce_scatter(out, process_group)
...
torch.fx.wrap("fused_dense_func")
torch.fx.wrap("fused_mlp_func")
