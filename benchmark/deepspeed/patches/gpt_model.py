...
def FP16CrossEntropy(output, labels):
    labels, loss_mask = labels[0], labels[1]
...
class GPTModelPipe(PipelineModule, MegatronModule):
    ...
    def __init__(self, config, num_tokentypes=0, parallel_output=True):
        ...
        if args.normalization == "layernorm":
            ...
            self.specs.append(
                LayerSpec(RMSNorm, args.hidden_size, args.layernorm_epsilon)
            )
        ...
        super().__init__(
            layers=self.specs,
            loss_fn=FP16CrossEntropy if args.fp16_lm_cross_entropy else CrossEntropy,
            topology=topo,
