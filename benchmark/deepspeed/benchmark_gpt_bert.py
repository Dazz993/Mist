...
df_config_map = {
    "1": ["ds_config_zero_stage_1.json"],
    "2": ["ds_config_zero_stage_2.json"],
    "3": ["ds_config_zero_stage_3.json"],
    "offload": ["ds_config_zero_offload.json"],
    "infinity": ["ds_config_zero_infinity_cpu.json"],
    "1-3": [
...
def generate_cases(
    ...
    for global_batch_size in global_batch_size_list:
        ...
        for parallelism_case in parallelism_cases:
            ...
            zero_choices = df_config_map[args.ds_stage]
            for zero in zero_choices:
                # Constraint 3: If zero offload is enabled, we can't do pipeline parallelism
                if not zero.startswith("ds_config_zero_stage") and pp > 1:
                    continue
                if zero != "ds_config_zero_stage_1.json" and dp == 1:
                    continue
                case = (
                    standard_model_name,
                    global_batch_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="1",
        choices=["1", "2", "3", "offload", "infinity", "1-3", "all"],
    )
